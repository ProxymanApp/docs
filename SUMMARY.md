# Table of contents

* [Overview](README.md)
* [Changelog](changelog.md)
* [License](license.md)
* [License Manager](license-manager.md)
* [Raycast](raycast.md)
* [Command-line](command-line.md)
* [MCP](mcp.md)
* [SKILL.md](skill-md.md)
* [Security Compliance](security-compliance.md)

## Team Workspace

* [Team Workspace](team-workspace/team-workspace.md)
* [Share Log online](team-workspace/share-log-online.md)

## Proxyman iOS

* [Proxyman for iOS](proxyman-ios/vpn-and-proxyman-certificate.md)
* [Map Local for iOS](proxyman-ios/map-local-for-ios.md)
* [Breakpoint for iOS](proxyman-ios/breakpoint-for-ios.md)
* [Tutorial for iOS](proxyman-ios/tutorial-for-ios/README.md)
  * [Map Local for iOS Tutorial](proxyman-ios/tutorial-for-ios/map-local-for-ios-tutorial.md)
  * [Breakpoint for iOS Tutorial](proxyman-ios/tutorial-for-ios/breakpoint-for-ios-tutorial.md)

## Debug on Devices <a href="#debug-devices" id="debug-devices"></a>

* [macOS](debug-devices/macos.md)
* [iOS Device](debug-devices/ios-device.md)
* [iOS Simulator](debug-devices/ios-simulator.md)
* [tvOS & watchOS](debug-devices/tvos-and-watchos.md)
* [Android Device & Emulator](debug-devices/android-device/README.md)
  * [Automatic Script for Android Emulator](debug-devices/android-device/automatic-script-for-android-emulator.md)
  * [Google-Play Android Emulator with Magisk](debug-devices/android-device/google-play-android-emulator-with-magisk.md)
  * [Sample Android Project](debug-devices/android-device/sample-android-project.md)
* [Firefox](debug-devices/firefox.md)
* [Java VMs](debug-devices/java.md)
* [Python](debug-devices/python.md)
* [Ruby](debug-devices/ruby.md)
* [NodeJS](debug-devices/nodejs.md)
* [Rust](debug-devices/rust.md)
* [Golang](debug-devices/golang.md)
* [React Native](debug-devices/react-native.md)
* [Flutter](debug-devices/flutter.md)
* [HTTP Clients](debug-devices/http-clients.md)
* [Docker](debug-devices/docker.md)
* [ElectronJS](debug-devices/electronjs.md)
* [NextJS (fetch)](debug-devices/nextjs-fetch.md)
* [Demo iOS & Android](debug-devices/demo-ios-and-android.md)

## Automatic Setup

* [Automatic Setup](automatic-setup/automatic-setup.md)
* [Manual Setup](automatic-setup/manual-setup.md)
* [Troubleshooting](automatic-setup/troubleshooting.md)

## Atlantis

* [Atlantis for iOS](atlantis/atlantis-for-ios.md)

## BASIC FEATURES

* [Proxyman Proxy Helper Tool](basic-features/proxy-setting-tool.md)
* [Request / Response Previewer](basic-features/request-response-viewer.md)
* [Connection Log](basic-features/connection-log.md)
* [SSL Proxying](basic-features/ssl-proxying.md)
* [HTTP/2](basic-features/http2.md)
* [Bypass Proxy List](basic-features/bypass-proxy-list.md)
* [Import / Export](basic-features/import-export.md)
* [Content Filter](basic-features/content-filter.md)
* [Tab View](basic-features/multiple-tabs.md)
* [Split View](basic-features/split-view.md)
* [Horizontal/Vertical/Window Layout](basic-features/horizontal-vertical-layout.md)
* [Copy as](basic-features/copy-as.md)
* [Custom Previewer Tab](basic-features/custom-previewer-tab.md)
* [Custom Header Column](basic-features/custom-header-column.md)
* [Regex (Regular Expression)](basic-features/regex.md)
* [Filter JSON Response](basic-features/filter-json-response.md)
* [Highlight by Color and Add Comment](basic-features/highlight-by-color-and-add-comment.md)
* [Import / Export Settings](basic-features/import-export-settings.md)
* [Multipart Form-Data Previewer](basic-features/multipart-form-data-previewer.md)
* [JSONPath](basic-features/jsonpaths.md)
* [JQ](basic-features/jq.md)
* [Customize Toolbar](basic-features/customize-toolbar.md)
* [Localization](basic-features/localization.md)
* [Quick Preview](basic-features/quick-preview.md)
* [Command Palette](basic-features/command-palette.md)

## ADVANCED FEATURES

* [Repeat](advanced-features/repeat.md)
* [Edit & Repeat](advanced-features/edit-and-repeat.md)
* [Compose new Request](advanced-features/compose.md)
* [No Caching](advanced-features/no-caching.md)
* [Breakpoint](advanced-features/breakpoint.md)
* [HTTP Message Templates](advanced-features/breakpoint-templates.md)
* [Map Local (File)](advanced-features/map-local.md)
* [Map Local (Directory)](advanced-features/map-local-directory.md)
* [Map Remote](advanced-features/map-remote.md)
* [External Proxy](advanced-features/external-proxy.md)
* [Save Session](advanced-features/save-session.md)
* [Protobuf](advanced-features/protobuf.md)
* [WebSocket](advanced-features/websocket.md)
* [Clear Session](advanced-features/clear-session.md)
* [Block List](advanced-features/blacklist.md)
* [Allow List](advanced-features/whitelist.md)
* [Charles Proxy Converter](advanced-features/charles-proxy-converter.md)
* [Custom Certificates](advanced-features/custom-certificates.md)
* [GraphQL](advanced-features/graphql.md)
* [Network Conditions](advanced-features/network-throttling.md)
* [Multiple Filters](advanced-features/multiple-filters.md)
* [Custom Filters](advanced-features/custom-filters.md)
* [Publish to Gist](advanced-features/publish-to-gist.md)
* [Reverse Proxy](advanced-features/reverse-proxy.md)
* [Code Generator](advanced-features/code-generator.md)
* [Diff](advanced-features/diff.md)
* [Access Control](advanced-features/access-control.md)
* [DNS Spoofing](advanced-features/dns-spoofing.md)
* [SOCKS Proxy](advanced-features/socks-proxy.md)
* [Swagger OpenAPI](advanced-features/swagger-openapi.md)
* [TLS Key Logging](advanced-features/tls-key-logging.md)

## Proxyman Windows

* [Install Certificate](proxyman-windows/install-certificate.md)
* [WSL](proxyman-windows/wsl.md)

## Scripting

* [Scripting](scripting/script.md)
* [async/await Request](scripting/async-await-request.md)
* [Addons](scripting/addons.md)
* [Built-in JS Libraries](scripting/built-in-js-libraries.md)
* [Use npm install](scripting/use-npm-install.md)
* [Write your own Addons](scripting/write-your-own-addons.md)
* [Snippet Code](scripting/snippet-code.md)
* [Environment Variables](scripting/environment-variables.md)

## Troubleshooting

* [Proxyman does not work with VPN apps](troubleshooting/proxyman-does-not-work-with-vpn-apps.md)
* [My Remote Devices (iOS/Android) could not connect to Proxyman?](troubleshooting/my-ios-devices-couldnt-connect-to-proxyman-via-proxy.md)
* [iOS 16 and iOS 17 issues](troubleshooting/ios-16-devices-issues.md)
* [SSL Error from HTTPS Request/Response](troubleshooting/get-ssl-error-from-https-request-and-response.md)
* [I could not see any requests from my localhost server](troubleshooting/couldnt-see-any-request-from-localhost-server.md)
* [I could not see any HTTP traffic from my NodeJS, Python, or Ruby scripts](troubleshooting/i-could-not-see-any-http-traffic-from-my-nodejs-python-or-ruby-scripts.md)
* [\*.local requests do not appear on Proxyman](troubleshooting/.local-doesnt-appear-in-proxyman.md)
* [I couldn't see any traffics on Proxyman](troubleshooting/i-couldnt-see-any-traffics-on-proxyman.md)
* [I couldn't see any requests from 3rd-party network libraries](troubleshooting/couldnt-see-any-requests-from-3rd-party-network-libraries.md)
* [\[Breakpoint\] Modify Request/Response by Raw Message](troubleshooting/breakpoint-modify-request-response-by-raw-message.md)
* [Could not change Proxyman App Icons](troubleshooting/could-not-change-proxyman-app-icons.md)
* [Lost data after updating Proxyman app?](troubleshooting/lost-data-after-updating-proxyman-app.md)
* [Proxyman consumes too much RAM & unresponsive](troubleshooting/proxyman-consumes-too-much-ram-and-unresponsive.md)
