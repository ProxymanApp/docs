---
description: "Connect Codex, Claude Code, or Cursor to Proxyman MCP to inspect captured HTTP/HTTPS traffic and create debugging rules from your AI assistant."
---

# MCP

Proxyman MCP connects your AI assistant to the Proxyman macOS app running on your computer. Use it to inspect captured requests and responses, investigate failures, and create debugging rules. Keep Proxyman open and capture the traffic you want to investigate before asking your assistant to analyze it.

## 1. What is Proxyman MCP?

**Proxyman MCP** (Model Context Protocol) is a feature that enables AI assistants (Codex, Claude Code, Cursor, and other MCP-compatible tools) to directly interact with the Proxyman macOS app. It allows AI to inspect HTTP traffic, create debugging rules, and control Proxyman - all through natural language conversations.
...
2. **Choose your MCP client:** In Proxyman Settings → MCP, select **Codex** or **Claude Code**, then use **Copy Command**. For **Cursor**, choose **Manual** and add the displayed configuration to Cursor's MCP settings.
3. Use the setup below for your client and installation. The command generated by Proxyman uses the path of your installed app, which may differ from these examples. Keep sensitive-data redaction enabled for normal debugging.
...
For Cursor and other MCP clients, add the configuration from Proxyman's **Manual** tab to the client's MCP settings. Select the configuration for your installed Proxyman app. These examples show the `mcpServers` entry:
...
* Ask your AI assistant to interact with Proxyman, for example: "Show the latest requests to api.example.com."
* If the client cannot find Proxyman, check that MCP is enabled and the configured app path is correct, then reconnect the client. If it connects but finds no flows, make a new request and use the [missing-traffic checklist](troubleshooting/i-couldnt-see-any-traffics-on-proxyman.md).
